...
3a. Rodar `docker compose exec -T app php artisan view:clear` (e equivalente em `documentation-app`) após qualquer rebuild que altere Blade/JS de layout. `storage/framework/views` fica em volume nomeado persistente (`app_storage`) que sobrevive ao `docker compose build`/recreate; se o mtime do `.blade.php` novo copiado pela imagem não for mais recente que o compilado já em cache, o Laravel serve a versão antiga silenciosamente — foi exatamente essa causa raiz observada no incidente de 2026-08-30 (toast/JS do layout não atualizava após rebuild).
